...
export default function Sidebar() {
  ...
  return (
    <>
        ...
        <nav className="flex-1 px-3 py-4 space-y-0.5" aria-label="Main navigation">
          {nav.map(({ href, label, icon: Icon }) => (
            <Link key={href} href={href}
              className={cn(
                'flex items-center gap-3 px-3 py-2 rounded-lg text-sm font-medium transition-colors',
                pathname === href ? 'bg-[#f0fafa] text-[#01696f]' : 'text-[#7a7974] hover:text-[#28251d] hover:bg-[#f3f0ec]'
              )}>
              <Icon size={16} />{label}
            </Link>
          ))}
      ...
      {/* Mobile top header */}
      <header className="md:hidden fixed top-0 left-0 right-0 z-40 bg-white border-b border-black/8 flex items-center justify-between px-4 h-12">
        <div className="flex items-center gap-2">
          <svg width="20" height="20" viewBox="0 0 32 32" fill="none" aria-label="Card Ops">
            <rect width="32" height="32" rx="8" fill="#01696f"/>
            <rect x="7" y="9" width="12" height="16" rx="2" fill="white" fillOpacity="0.9"/>
            <rect x="13" y="7" width="12" height="16" rx="2" fill="white" fillOpacity="0.4"/>
            <path d="M10 14h6M10 17h4" stroke="#01696f" strokeWidth="1.5" strokeLinecap="round"/>
        ...
        <button onClick={signOut} className="p-2 rounded-lg text-[#7a7974] hover:text-[#28251d] hover:bg-[#f3f0ec] transition-colors">
          <LogOut size={16} />
        </button>
      </header>

      {/* Mobile bottom nav */}
      <nav className="md:hidden fixed bottom-0 left-0 right-0 z-50 bg-white border-t border-black/8 flex" style={{ paddingBottom: 'env(safe-area-inset-bottom)' }} aria-label="Mobile navigation">
        {nav.map(({ href, label, icon: Icon }) => (
          <Link key={href} href={href}
            className={cn(
              'flex-1 flex flex-col items-center gap-1 py-3 text-[10px] font-medium transition-colors min-h-[56px] justify-center',
